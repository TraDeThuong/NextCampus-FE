  ...
  if (isError || !meetingData) {
    return (
      <div className="flex flex-col items-center gap-3 py-16 text-center">
        ...
        <p className="text-sm text-slate-400">{t("admin.meetings.loadDetailError")}</p>
      </div>
    );
  }
    ...
    <div className="space-y-5">
      ...
      <div>
        <div className="flex items-start justify-between gap-3">
          ...
          <span
            className={`shrink-0 rounded-full px-3 py-0.5 text-xs font-medium mr-20 ${STATUS_BADGE[meeting.status] || STATUS_BADGE.DRAFT}`}
          >
            {meeting.status}
          </span>
        </div>
      ...
      <div className="flex flex-wrap gap-x-8 gap-y-1 text-xs text-slate-500">
        <span>
          {t("admin.meetings.host")} <span className="text-slate-300">{meeting.host.fullName || meeting.host.email}</span>
        </span>
        <span>
          {t("admin.meetings.createdBy")}{" "}
          <span className="text-slate-300">{meeting.creator.fullName || meeting.creator.email}</span>
        </span>
      </div>
                  ...
                  {a.status === "PENDING" ? (
                    <div className="flex shrink-0 items-center gap-1.5">
                      <button
                        ...
                        className="rounded-lg bg-emerald-500/20 px-2.5 py-1 text-xs font-medium text-emerald-300 transition hover:bg-emerald-500/30"
                      >
                        Approve
                      </button>
                      <button
          ...
          {confirmAction ? (
                ...
                <div className="flex-1">
                  ...
                  <div className="mt-3 flex items-center gap-2">
                    <button
                      ...
                      className="hover:cursor-pointer rounded-lg border border-white/10 bg-white/5 px-3 py-1.5 text-xs text-slate-300 transition hover:text-white"
                    >
                      No, keep it
                    </button>
                    <button
            ...
            <div className="flex items-center justify-end gap-2">
              ...
              >
                ...
                <span className="ml-1">Cancel Meeting</span>
              </Button>
            </div>
          )}
        </div>
