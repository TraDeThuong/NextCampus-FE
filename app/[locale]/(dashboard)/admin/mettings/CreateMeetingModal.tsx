  ...
  return (
    <div className="px-1 py-4">
      <div className="mb-6 flex items-center gap-3">
        ...
        <div>
          <h3 className="text-base font-semibold text-white">{t("admin.meetings.scheduleMeeting")}</h3>
          <p className="text-xs text-slate-500">
            {watchVisibility === "TEAM"
              ? t("admin.meetings.visibleToAll")
              : selectedLeaderIds.length > 0
                ? t(selectedLeaderIds.length > 1 ? "admin.meetings.leadersInvited" : "admin.meetings.leaderInvited", { n: selectedLeaderIds.length })
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
        {/* Title */}
        <div>
          <label className={labelClass}>{t("admin.meetings.formTitle")}</label>
          <input
            type="text"
        ...
        {/* Description */}
        <div>
          <label className={labelClass}>{t("admin.meetings.formDescription")}</label>
          <textarea
            rows={2}
        ...
        {/* Host (auto) */}
        <div>
          <label className={labelClass}>{t("admin.meetings.formHost")}</label>
          <div className="mt-1 flex items-center gap-2 rounded-xl border border-white/10 bg-white/5 px-4 py-2.5">
            ...
            <span className="text-sm text-slate-300">
              {currentUser?.fullName || currentUser?.email || t("admin.meetings.formAuto")}
            </span>
        ...
        {/* Meeting Type */}
        <div>
          <label className={labelClass}>{t("admin.meetings.formMeetingType")}</label>
        ...
        {/* Location */}
        <div>
          <label className={labelClass}>{t("admin.meetings.formLocation")}</label>
          <input
            type="text"
        ...
        {(watchMeetingType === "ONLINE" || watchMeetingType === "HYBRID") && (
          <div>
            <label className={labelClass}>{t("admin.meetings.formMeetingLink")}</label>
            <input
              type="url"
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            <label className={labelClass}>{t("admin.meetings.formStartTime")}</label>
            <input
              type="datetime-local"
              {...register("startTime")}
              className={`${inputClass} mt-1`}
          ...
          <div>
            <label className={labelClass}>{t("admin.meetings.formEndTime")}</label>
            <input
              type="datetime-local"
              {...register("endTime")}
              className={`${inputClass} mt-1`}
        ...
        {watchVisibility === "PRIVATE" && (
          <div>
            <label className={labelClass}>
              ...
              {t("admin.meetings.inviteLeaders")}
            </label>
            {leaders.length === 0 ? (
            ...
            {selectedLeaderIds.length > 0 && (
              <p className="mt-1 text-xs text-slate-500">
                {selectedLeaderIds.length > 1 ? t("admin.meetings.leadersSelectedPlural", { n: selectedLeaderIds.length }) : t("admin.meetings.leadersSelected", { n: selectedLeaderIds.length })}
              </p>
            )}
          </div>
        )}
        ...
        {/* Visibility */}
        <div>
          <label className={labelClass}>{t("admin.meetings.formVisibility")}</label>
          <div className="mt-2 flex gap-3">
            {(["TEAM", "PRIVATE"] as MeetingVisibility[]).map((v) => (
              ...
              >
                <input
                  ...
                  className="accent-primary-main"
                />
                {v === "TEAM" ? t("admin.meetings.formCompany") : t("admin.meetings.formPrivate")}
              </label>
            ))}
          </div>
        </div>
        ...
        {/* Status */}
        <div>
          <label className={labelClass}>{t("admin.meetings.formStatus")}</label>
          <div className="mt-2 flex gap-3">
