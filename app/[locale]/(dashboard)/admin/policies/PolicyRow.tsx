...
export default function PolicyRow({ policy }: PolicyRowProps) {
  ...
  const formattedCreatedDate = new Date(policy.createdAt).toLocaleDateString("en-US", { year: "numeric", month: "short", day: "numeric" });
  const formattedUpdatedDate = new Date(policy.updatedAt).toLocaleDateString("en-US", { year: "numeric", month: "short", day: "numeric" });
    ...
    <Modal>
      <Table.Row>
        <div className="flex flex-col min-w-0 pr-4">
          ...
          <span className="text-xs text-slate-500 line-clamp-1 mt-0.5">{policy.content.replace(/<[^>]*>/g, "")}</span>
        </div>
      ...
      <Modal.Window name={`edit-policy-${policy.id}`} size="md">
        <EditPolicyForm policy={policy} isPending={updating} onSubmit={(data) => updatePolicy({ id: policy.id, payload: data })} />
      </Modal.Window>

      <Modal.Window name={`delete-policy-${policy.id}`} size="sm">
        <DeleteConfirm title={policy.title} isPending={deleting} onConfirm={(onCloseModal) => { deletePolicy(policy.id, { onSuccess: () => onCloseModal?.() }); }} />
      </Modal.Window>
    </Modal>
  );
}

function EditPolicyForm({ policy, isPending, onSubmit, onCloseModal }: { policy: Regulation; isPending: boolean; onSubmit: (data: FormValues) => void; onCloseModal?: () => void }) {
  const t = useTranslations();
  const { register, handleSubmit, control, formState: { errors } } = useForm<FormValues>({ resolver: zodResolver(policySchema), defaultValues: { title: policy.title, content: policy.content } });
  const inputClass = "w-full rounded-xl border border-white/10 bg-white/5 py-3 px-4 text-sm text-white outline-none transition focus:border-cyan-400/50 placeholder:text-slate-600";

  return (
    <div className="px-2 py-4">
      <div className="flex items-center gap-3 border-b border-white/10 pb-4">
        <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-cyan-500/10 text-cyan-400"><Edit2 className="h-5 w-5" /></div>
        <div>
          <h3 className="text-lg font-semibold text-white">{t("admin.policies.editTitle")}</h3>
          <p className="text-xs text-slate-400">{t("admin.policies.editDesc")}</p>
        </div>
      </div>
      <form onSubmit={handleSubmit((data) => { onSubmit(data); onCloseModal?.(); })} className="mt-6 space-y-4">
        <div className="space-y-1">
          <label className="text-xs font-medium text-slate-300">{t("admin.policies.policyTitle")}</label>
          <input type="text" {...register("title")} className={inputClass} />
          {errors.title && <p className="text-xs text-red-400">{errors.title.message}</p>}
        </div>
        <div className="space-y-1">
          <label className="text-xs font-medium text-slate-300">{t("admin.policies.content")}</label>
          <Controller control={control} name="content" render={({ field }) => (<RichTextEditor value={field.value} onChange={field.onChange} placeholder={t("admin.policies.editContentPlaceholder")} />)} />
          {errors.content && <p className="text-xs text-red-400">{errors.content.message}</p>}
        </div>
        <div className="flex justify-end gap-3 pt-4 border-t border-white/5 mt-6">
          <button type="button" onClick={onCloseModal} disabled={isPending} className="rounded-xl border border-white/10 bg-white/5 px-5 py-2 text-sm text-slate-300 transition hover:text-white disabled:opacity-50">
            {t("admin.policies.cancel")}
          </button>
          <Button type="submit" disabled={isPending} variant="glass">
            {isPending ? <Loader2 className="h-4 w-4 animate-spin" /> : t("admin.policies.saveChanges")}
          </Button>
        </div>
      </form>
    </div>
...
function DeleteConfirm({ title, isPending, onConfirm, onCloseModal }: { title: string; isPending: boolean; onConfirm: (close?: () => void) => void; onCloseModal?: () => void }) {
  const t = useTranslations();
  return (
    <div className="px-2 py-8 text-center">
      <div className="mx-auto flex h-12 w-12 items-center justify-center rounded-full bg-red-500/10 text-red-400"><Trash2 className="h-6 w-6" /></div>
      <h3 className="mt-4 text-base font-semibold text-white">{t("admin.policies.deleteTitle")}</h3>
      <p className="mt-2 text-sm text-slate-400 max-w-sm mx-auto" dangerouslySetInnerHTML={{ __html: t("admin.policies.deleteConfirm", { title }) }} />
      <div className="flex justify-center gap-3 pt-6">
        <button type="button" onClick={onCloseModal} disabled={isPending} className="rounded-xl border border-white/10 bg-white/5 px-5 py-2 text-sm text-slate-300 transition hover:text-white disabled:opacity-50">
          {t("admin.policies.cancel")}
        </button>
        <button onClick={() => onConfirm(onCloseModal)} disabled={isPending} className="rounded-xl bg-red-600 hover:bg-red-500 px-5 py-2 text-sm text-white transition disabled:opacity-50 flex items-center justify-center">
          {isPending ? <Loader2 className="h-4 w-4 animate-spin" /> : t("admin.policies.confirmDelete")}
        </button>
      </div>
    </div>
  );
