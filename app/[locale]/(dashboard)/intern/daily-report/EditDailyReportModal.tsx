...
export default function EditDailyReportModal({ report, onClose }: Props) {
  ...
  const [videoLink, setVideoLink] = useState(report.videoDemo ?? ""); const [newAttachments, setNewAttachments] = useState<File[]>([]);
  ...
  async function handleSubmit(e: React.FormEvent) { e.preventDefault(); if (isSubmitting) return; setIsSubmitting(true);
    try { await updateDailyReport.mutateAsync({ id: report.id, payload: { content, prLink: prLink.trim() || null, videoDemo: videoLink.trim() || null } }); for (const file of newAttachments) await uploadAttachment.mutateAsync({ reportId: report.id, file }); onClose(); } catch (err) { console.error(err); } finally { setIsSubmitting(false); }
  }
      ...
      <div onClick={(e) => e.stopPropagation()} className="relative w-full max-w-lg overflow-hidden rounded-[32px] border border-white/10 bg-[linear-gradient(145deg,rgba(255,255,255,0.08),rgba(255,255,255,0.02))] shadow-[0_25px_80px_rgba(0,0,0,0.55)]">
        ...
        <div className="relative p-6">
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div><label className="mb-1.5 flex items-center gap-2 text-sm font-medium text-slate-200"><Video className="h-4 w-4 text-cyan-400" />{tm("videoDemo")}</label><input type="url" value={videoLink} onChange={(e) => setVideoLink(e.target.value)} placeholder={tm("videoPlaceholder")} disabled={isPending} className="w-full rounded-xl border border-white/10 bg-white/5 px-4 py-2.5 text-sm text-white placeholder:text-slate-500 outline-none transition focus:border-cyan-400/50 disabled:opacity-50" /></div>
            ...
            <div>
              ...
              <div className="space-y-2">
                ...
                <input type="file" multiple disabled={isPending} onChange={(e) => { const files = Array.from(e.target.files ?? []); setNewAttachments((prev) => [...prev, ...files]); }} className="w-full rounded-xl border border-white/10 bg-white/5 px-4 py-2.5 text-sm text-slate-400 file:mr-3 file:rounded-lg file:border-0 file:bg-cyan-500/10 file:px-3 file:py-1 file:text-xs file:text-cyan-300 file:cursor-pointer outline-none disabled:opacity-50" />
              </div>
            </div>
