...
export default function TaskSubmissionModal({ assignmentId, submission, readOnly, onClose }: Props) {
  ...
  const uploadVideo = useUploadSubmissionVideo(); const isEdit = !!submission;
  ...
  function handleVideoFileChange(file: File | undefined) {
    ...
    if (exceedsUploadLimit(file, UPLOAD_LIMITS_MB.submissionVideo)) { setVideoFile(null); toast.error(tm("videoSizeError", { limit: UPLOAD_LIMITS_MB.submissionVideo })); return; }
    setVideoFile(file);
  }
      ...
      <div onClick={(e) => e.stopPropagation()} className="relative w-full max-w-lg overflow-hidden rounded-[32px] border border-white/10 bg-[linear-gradient(145deg,rgba(255,255,255,0.08),rgba(255,255,255,0.02))] shadow-[0_25px_80px_rgba(0,0,0,0.55)]">
        ...
        <div className="relative p-6">
          <div className="mb-6 flex items-start justify-between">
            <div>
              <h2 className="text-xl font-bold flex items-center gap-2">{isEdit ? <Pencil className="h-5 w-5 text-amber-400 shrink-0" /> : <Send className="h-5 w-5 text-cyan-400 shrink-0" />}<span className="metal-text">{isEdit ? tm("editTitle") : tm("submitTitle")}</span></h2>
              <p className="mt-1 text-sm text-muted">{isEdit ? tm("editDesc") : tm("submitDesc")}</p>
            </div>
