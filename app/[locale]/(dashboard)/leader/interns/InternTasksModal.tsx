...
export default function InternTasksModal({ intern, onClose }: Props) {
  ...
  return (
    <div onClick={onClose} className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-950/80 p-4 backdrop-blur-md">
      <div onClick={(e) => e.stopPropagation()} className="relative flex w-full max-w-5xl h-[80vh] flex-col overflow-hidden rounded-2xl border border-slate-700/60 bg-gradient-to-b from-slate-900 via-slate-900 to-slate-950 shadow-[0_0_50px_rgba(0,0,0,0.8),inset_0_1px_1px_rgba(255,255,255,0.05)] text-slate-100">
        ...
        <div className="relative flex items-center justify-between border-b border-slate-800/80 px-6 py-4 bg-slate-900/40 backdrop-blur-sm">
          <div>
            ...
            <p className="mt-0.5 text-xs font-mono text-slate-500">
              [ {t("tasksModalTotal", { count: assignments.length })} ]
            </p>
          </div>
          <button onClick={onClose} className="group flex h-9 w-9 items-center justify-center rounded-lg border border-slate-800 bg-slate-900 transition-all duration-300 hover:border-slate-600 hover:bg-slate-800 active:scale-95 shadow-inner">
        ...
        <div className="flex flex-1 overflow-hidden relative">
          {listLoading ? (
            <div className="flex w-full items-center justify-center"><Spinner size="lg" /></div>
            ...
            <div className="flex w-full flex-col items-center justify-center py-12 text-slate-500">
              ...
              <p className="text-sm font-mono tracking-wide">{t("tasksModalEmpty")}</p>
            </div>
          ) : (
            <>
              <div className="w-2/5 border-r border-slate-800/80 overflow-y-auto p-4 space-y-2 bg-slate-950/40 custom-scrollbar">
                <div className="px-2 pb-2 text-[10px] font-bold tracking-widest text-slate-500 uppercase font-mono">{t("tasksModalRegistry")}</div>
                {assignments.map((a) => (
                  <TaskRowButton key={a.id} assignment={a} isSelected={selectedTaskId === a.taskId} onClick={() => setSelectedTaskId(selectedTaskId === a.taskId ? null : a.taskId)} />
                ))}
              </div>
              ...
              <div className="w-3/5 overflow-y-auto p-6 bg-slate-900/20 custom-scrollbar relative">
                {selectedTaskId ? (
                  taskLoading ? (
                    ...
                    <p className="text-center text-sm font-mono text-red-400/80 py-12">{t("tasksModalFetchError")}</p>
                  )
                ) : (
                  <div className="flex h-full flex-col items-center justify-center text-slate-600">
                    <Layers className="h-12 w-12 stroke-[1] mb-2 opacity-20 animate-pulse" />
                    <p className="text-xs font-mono tracking-wider uppercase">{t("tasksModalSelectHint")}</p>
                  </div>
                )}
              </div>
            </>
...
function TaskRowButton({ assignment, isSelected, onClick }: { assignment: TaskAssignment; isSelected: boolean; onClick: () => void }) {
  const statusColors: Record<string, string> = {
    DONE: "bg-emerald-400/20", IN_PROGRESS: "bg-blue-400/20", REVIEW: "bg-purple-400/20",
    TODO: "bg-slate-700/40", BLOCKED: "bg-red-400/20", PENDING_APPROVAL: "bg-amber-400/20",
  };
  ...
  return (
    <button onClick={onClick} className={`group relative flex w-full items-center justify-between rounded-xl border p-3.5 text-left transition-all duration-200 ${isSelected ? "border-cyan-500/50 bg-gradient-to-r from-slate-900 to-slate-800/80 shadow-[0_4px_20px_rgba(0,0,0,0.4),inset_0_1px_0_rgba(255,255,255,0.05)] text-white" : "border-slate-850 bg-slate-900/40 text-slate-400 hover:border-slate-700 hover:bg-slate-900/80"}`}>
      ...
      <div className="pl-2 space-y-1 overflow-hidden pr-2">
        <div className="font-mono text-xs font-bold tracking-wider text-slate-300 group-hover:text-cyan-400 transition-colors">{assignment.task.code || "UNTITLED-UNIT"}</div>
...
function TaskDetailPanel({ task, assignment }: { task: NonNullable<ReturnType<typeof useTask>["data"]>["data"]; assignment: TaskAssignment | undefined }) {
  const t = useTranslations("leader.interns");
    ...
    <div className="space-y-6">
      <div className="space-y-2 border-b border-slate-800/60 pb-4">
        <div className="flex flex-wrap items-center gap-2">
          <span className="font-mono text-xs bg-slate-800 border border-slate-700 text-slate-300 px-2 py-0.5 rounded-md shadow-inner">{task.code ?? "N/A"}</span>
          {assignment && (
          ...
          <span className={`inline-flex rounded-md border px-2 py-0.5 text-xs font-mono uppercase font-bold tracking-wider ${priorityBadge[task.priority] ?? ""}`}>{task.priority}</span>
        </div>
      ...
      <div className="grid grid-cols-2 gap-3 bg-slate-950/40 border border-slate-850 p-4 rounded-xl shadow-inner">
        <DetailGridRow icon={Calendar} label={t("tasksModalTargetDeadline")} value={new Date(task.deadline).toLocaleDateString("en-GB")} />
        <DetailGridRow icon={User} label={t("tasksModalAssignedBy")} value={task.creator.fullName ?? task.creator.email} />
        <DetailGridRow icon={Layers} label={t("tasksModalGroup")} value={task.taskGroup?.name ?? "—"} />
        <DetailGridRow icon={Calendar} label={t("tasksModalActivation")} value={task.startDate ? new Date(task.startDate).toLocaleDateString("en-GB") : "—"} />
        {task.estDays && (
          <div className="col-span-2 mt-1 pt-2 border-t border-slate-850">
            <DetailGridRow label={t("tasksModalEstDuration")} value={t("tasksModalSystemDays", { days: task.estDays })} />
          </div>
        )}
      </div>
      ...
      <div className="space-y-4 font-sans">
        {task.description && (
          <div className="space-y-1.5">
            <h4 className="text-[10px] font-bold uppercase font-mono tracking-widest text-slate-500">{t("tasksModalObjective")}</h4>
            <div className="rounded-xl border border-slate-850 bg-slate-900/30 p-3.5 text-sm text-slate-400 leading-relaxed">{task.description}</div>
          </div>
        )}
        {task.acceptanceCriteria && (
          <div className="space-y-1.5">
            <h4 className="text-[10px] font-bold uppercase font-mono tracking-widest text-slate-500">{t("tasksModalCriteria")}</h4>
            <div className="rounded-xl border border-slate-850 bg-slate-900/30 p-3.5 text-sm text-slate-400 leading-relaxed font-mono whitespace-pre-line">{task.acceptanceCriteria}</div>
          </div>
        )}
        {task.taskNotes && (
          <div className="space-y-1.5">
            <h4 className="text-[10px] font-bold uppercase font-mono tracking-widest text-slate-500">{t("tasksModalNotes")}</h4>
            <div className="rounded-xl border border-slate-850 bg-slate-950/40 p-3.5 text-sm text-amber-400/80 leading-relaxed italic">{task.taskNotes}</div>
          </div>
        )}
      </div>
      ...
      {task.attachments.length > 0 && (
        <div className="space-y-2 pt-2">
          <h4 className="text-[10px] font-bold uppercase font-mono tracking-widest text-slate-500">{t("tasksModalVaults", { count: task.attachments.length })}</h4>
          <div className="grid grid-cols-1 gap-2">
            {task.attachments.map((att) => (
              <a key={att.id} href={att.fileUrl} target="_blank" rel="noopener noreferrer" className="group flex items-center justify-between rounded-xl border border-slate-800 bg-slate-900/50 px-4 py-2.5 text-xs text-slate-300 transition-all hover:border-cyan-500/40 hover:bg-slate-900 hover:text-cyan-400">
                ...
                <span className="text-[10px] font-mono text-slate-600 group-hover:text-cyan-500 uppercase border border-slate-800 px-1.5 py-0.5 rounded bg-slate-950/60 transition-colors ml-2 shrink-0">{t("tasksModalOpenExternal")}</span>
              </a>
            ))}
          </div>
        </div>
...
function DetailGridRow({ icon: Icon, label, value }: { icon?: React.ComponentType<{ className?: string }>; label: string; value: React.ReactNode }) {
  return (
