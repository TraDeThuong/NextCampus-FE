...
const COLUMNS = "minmax(100px,1fr) minmax(50px,0.5fr) minmax(70px,0.5fr) 100px 100px 100px 150px 40px";
...
export default function LeaderInternTable() {
  ...
  const progressMap = useMemo(() => {
    ...
    for (const [internId, counts] of byIntern) {
      ...
      map.set(internId, { internId, internName: "", internEmail: "", completedTasks: counts.done, totalTasks: counts.total, avgScore: 0, overdueCount: counts.overdue, healthStatus });
    }
    return map;
  }, [assignments]);
  ...
  if (isPending) {
    return <MetalCard className="flex items-center justify-center py-20"><Spinner size="lg" /></MetalCard>;
  }
  ...
  if (isError) {
    return (
      <MetalCard className="flex flex-col items-center justify-center gap-3 py-20">
        ...
        <p className="text-sm text-slate-400">{t("loadError")}</p>
      </MetalCard>
    );
  }
  ...
  if (interns.length === 0) {
    return (
      <MetalCard className="flex flex-col items-center justify-center gap-3 py-20">
        <p className="text-sm text-slate-500">{t("noInterns")}</p>
      </MetalCard>
    );
  }
  ...
  return (
    ...
    >
      ...
      <Table.Body
        data={interns}
        render={(intern) => (
          <LeaderInternRow key={intern.id} intern={intern} taskProgress={progressMap.get(intern.id)} />
        )}
      />
