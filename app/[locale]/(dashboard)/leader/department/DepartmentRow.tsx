...
function ManagePositions({ department }: { department: Department }) {
  ...
  const handleAdd = (e: React.FormEvent) => {
    ...
    if (!trimmed) return;
    createPosition({ name: trimmed, departmentId: department.id }, {
      onSuccess: () => { setNewPositionName(""); setOriginalNewPosTyped(""); },
    });
  };

  const handleAddPosKeyDown = (e: React.KeyboardEvent<HTMLInputElement>) => {
    if (e.key === "Tab") {
      const matches = predefinedForDept.filter((p) => p.toLowerCase().includes(originalNewPosTyped.toLowerCase()));
      if (matches.length > 0) {
        e.preventDefault();
        if (matches.length === 1) { setNewPositionName(matches[0]); }
        else { const index = addPosSuggestIdx % matches.length; setNewPositionName(matches[index]); setAddPosSuggestIdx(index + 1); }
      }
    } else if (e.key === "Enter") {
      e.preventDefault();
  ...
  const handleEditPosKeyDown = (e: React.KeyboardEvent<HTMLInputElement>, posId: string, originalName: string) => {
    if (e.key === "Tab") {
      const matches = predefinedForDept.filter((p) => p.toLowerCase().includes(originalEditPosTyped.toLowerCase()));
      if (matches.length > 0) {
        e.preventDefault();
        if (matches.length === 1) { setEditingPosName(matches[0]); }
        else { const index = editPosSuggestIdx % matches.length; setEditingPosName(matches[index]); setEditPosSuggestIdx(index + 1); }
      }
    } else if (e.key === "Enter") {
      if (editingPosName.trim() && editingPosName.trim() !== originalName) {
        updatePosition({ id: posId, payload: { name: editingPosName.trim() } });
      }
                ...
                {editingPosId === pos.id ? (
                  <div className="flex-1 relative">
                    <input
                      ...
                      onChange={(e) => { setEditingPosName(e.target.value); setOriginalEditPosTyped(e.target.value); setEditPosSuggestIdx(0); }}
                      onKeyDown={(e) => handleEditPosKeyDown(e, pos.id, pos.name)}
                      onBlur={() => {
                        if (editingPosName.trim() && editingPosName.trim() !== pos.name) {
                          updatePosition({ id: pos.id, payload: { name: editingPosName.trim() } });
                        }
                        setEditingPosId(null);
                      }}
