...
export default function MeetingDetailModal({ meetingId, onCloseModal }: Props) {
  const t = useTranslations("leader.meetings.detailModal");
    ...
    <div className="space-y-5">
      <div>
        <div className="flex items-start justify-between gap-3">
          ...
          <span className={`shrink-0 rounded-full px-3 py-0.5 text-xs font-medium mr-20 ${STATUS_BADGE[meeting.status] || STATUS_BADGE.DRAFT}`}>{meeting.status}</span>
        </div>
        {meeting.description && <p className="mt-2 text-sm leading-relaxed text-slate-400">{meeting.description}</p>}
      </div>
        ...
        <div className="rounded-2xl border border-white/5 bg-white/[0.03] p-4">
          {showLeaveForm ? (
            <div className="space-y-3">
              <p className="text-sm font-medium text-slate-200">{myStatus === "ACCEPTED" ? t("leaveMeeting") : t("declineInvitation")}</p>
              <textarea rows={2} placeholder={t("reasonPlaceholder")} value={leaveReason} onChange={(e) => setLeaveReason(e.target.value)} className="w-full rounded-xl border border-white/10 bg-white/5 px-4 py-2 text-sm text-white outline-none transition focus:border-primary-main/50 placeholder:text-slate-600 resize-none" />
              <div className="flex items-center gap-2">
                <button type="button" onClick={() => { setShowLeaveForm(false); setLeaveReason(""); }} className="rounded-lg border border-white/10 bg-white/5 px-3 py-1.5 text-xs text-slate-300 transition hover:text-white">{t("cancel")}</button>
                <button type="button" onClick={handleDecline} disabled={!leaveReason.trim() || rsvpMeeting.isPending || submitAbsence.isPending} className="rounded-lg bg-red-500/20 px-3 py-1.5 text-xs font-medium text-red-300 transition hover:bg-red-500/30 disabled:opacity-50">{t("confirm")}</button>
              </div>
            </div>
          ) : (
      ...
      <div className="flex flex-wrap gap-x-8 gap-y-1 text-xs text-slate-500">
        <span>{t("host")} <span className="text-slate-300">{meeting.host.fullName || meeting.host.email}</span></span>
        <span>{t("createdBy")} <span className="text-slate-300">{meeting.creator.fullName || meeting.creator.email}</span></span>
      </div>

      {meeting.participants.length > 0 && (
        <div>
          <h4 className="mb-2 text-xs font-semibold uppercase tracking-wider text-slate-500">{t("participants")}</h4>
          <div className="max-h-[200px] space-y-1 overflow-y-auto rounded-xl border border-white/5 bg-white/[0.02] p-2">
            {meeting.participants.map((p) => { const { Icon, color } = INVITATION_ICON[p.invitationStatus] || INVITATION_ICON.PENDING; return (
              <div key={p.id} className="flex items-center gap-2 rounded-lg px-2 py-1.5 text-sm"><Icon className={`h-3.5 w-3.5 shrink-0 ${color}`} /><span className="flex-1 truncate text-slate-300">{p.user.fullName || p.user.email}</span><span className="shrink-0 text-xs text-slate-600">{p.participantRole}</span></div>
            );})}
          </div>
        </div>
      )}
      ...
      {absences.length > 0 && (
        <div>
          <h4 className="mb-2 text-xs font-semibold uppercase tracking-wider text-slate-500">{t("absenceRequests")}</h4>
          <div className="space-y-2">
            {absences.map((a) => (
              <div key={a.id} className="rounded-xl border border-white/5 bg-white/[0.02] p-3">
                <div className="flex items-start justify-between gap-2">
                  <div className="min-w-0 flex-1">
                    <p className="text-sm font-medium text-slate-200">{a.participant.user.fullName || a.participant.user.email}</p>
                    <p className="mt-0.5 text-xs text-slate-400">{a.reason}</p>
                    {a.reviewNote && <p className="mt-0.5 text-xs italic text-slate-500">{t("note")} {a.reviewNote}</p>}
                  </div>
                  {a.status === "PENDING" ? (
                    <div className="flex shrink-0 items-center gap-1.5">
                      <button type="button" onClick={() => handleReview(a.id, "APPROVED")} disabled={reviewAbsence.isPending} className="rounded-lg bg-emerald-500/20 px-2.5 py-1 text-xs font-medium text-emerald-300 transition hover:bg-emerald-500/30">{t("approve")}</button>
                      <button type="button" onClick={() => handleReview(a.id, "REJECTED")} disabled={reviewAbsence.isPending} className="rounded-lg bg-red-500/20 px-2.5 py-1 text-xs font-medium text-red-300 transition hover:bg-red-500/30">{t("reject")}</button>
                    </div>
                  ) : (
                    <span className={`shrink-0 rounded-full px-2 py-0.5 text-xs font-medium ${a.status === "APPROVED" ? "bg-emerald-500/20 text-emerald-300" : "bg-red-500/20 text-red-300"}`}>{a.status}</span>
                  )}
                </div>
              </div>
            ))}
        ...
        <div className="border-t border-white/10 pt-4">
          {confirmAction ? (
            <div className="rounded-2xl border border-red-500/20 bg-red-500/5 p-4">
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1">
                  <p className="text-sm font-medium text-red-300">{t("cancelMeetingTitle")}</p>
                  <p className="mt-1 text-xs text-slate-400">{t("cancelMeetingDesc")}</p>
                  <div className="mt-3 flex items-center gap-2">
                    <button type="button" onClick={() => setConfirmAction(null)} className="hover:cursor-pointer rounded-lg border border-white/10 bg-white/5 px-3 py-1.5 text-xs text-slate-300 transition hover:text-white">{t("noKeepIt")}</button>
                    <button type="button" onClick={handleCancel} className="hover:cursor-pointer rounded-lg bg-red-500/20 px-3 py-1.5 text-xs font-medium text-red-300 transition hover:bg-red-500/30">{t("yesCancelIt")}</button>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="flex items-center justify-end gap-2">
              <Button variant="glass" size="sm" onClick={() => setConfirmAction("cancel")} disabled={updateMeeting.isPending}>
                {updateMeeting.isPending ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <XCircle className="h-3.5 w-3.5" />}<span className="ml-1">{t("cancelMeeting")}</span>
              </Button>
            </div>
          )}
        </div>
