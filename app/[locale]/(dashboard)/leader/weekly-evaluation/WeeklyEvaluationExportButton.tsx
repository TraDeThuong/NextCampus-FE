...
export default function WeeklyEvaluationExportButton({ id }: Props) {
  ...
  return (
    <Button variant="glass" size="sm" onClick={() => mutate(id)} isLoading={isPending} className="flex items-center gap-1.5">
      <FileDown className="h-4 w-4 mr-1" /><span>{t("exportPdf")}</span>
    </Button>
  );
}
