...
import { FileText, Link, Video, ExternalLink, AlertCircle, Paperclip, Download } from "lucide-react";
...
export default function ReportDetail({ report, isLoading, missingDate, onEdit }: Props) {
  const { data: attachmentsData } = useReportAttachments(
    report?.id,
  );
      ...
      <MetalCard>
        <div className="rounded-3xl p-8 flex flex-col items-center justify-center min-h-[300px] text-center">
          ...
          <p className="text-sm text-slate-500 mt-1">
            You have not submitted a daily report for this date.
          </p>
        </div>
      </MetalCard>
    );
      ...
      <MetalCard>
        <div className="rounded-3xl p-8 flex flex-col items-center justify-center min-h-[300px] text-center">
          ...
          <p className="text-sm text-slate-600 max-w-xs">
            Click on a highlighted date in the calendar to view the daily report
            details.
          </p>
        </div>
      </MetalCard>
  ...
  const createdTime = new Date(report.createdAt).toLocaleTimeString("en-GB", {
    hour: "2-digit",
    minute: "2-digit",
  });
