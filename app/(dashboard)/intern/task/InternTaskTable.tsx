      ...
      <div className="relative flex items-center justify-between border-b border-slate-800/80 px-6 py-4 bg-slate-900/40 backdrop-blur-sm">
        <div>
          ...
          <p className="mt-0.5 text-xs font-mono text-slate-500">
            [ Total: {sortedAssignments.length} UNIT{sortedAssignments.length !== 1 ? "S" : ""} ]
          </p>
        </div>
      </div>
            ...
            <div className="w-3/4 overflow-y-auto p-6 bg-slate-900/20 relative custom-scrollbar">
              {assignmentId ? (
                ...
                <div className="flex h-full flex-col items-center justify-center text-slate-600">
                  <Layers className="h-12 w-12 stroke-[1] mb-2 opacity-20" />
...
function TaskRowButton({
  ...
  return (
    ...
    >
      <div
        className={`absolute left-0 top-1/4 h-1/2 w-[3px] rounded-r-full transition-all ${
...
function TaskDetailPanel({
  ...
  task: NonNullable<ReturnType<typeof useTask>["data"]>["data"] | null;
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="space-y-2 border-b border-slate-800/60 pb-4">
        <div className="flex items-center justify-between gap-3">
          <div className="flex flex-wrap items-center gap-2">
            ...
            {assignment && (
              <span
                className={`inline-flex items-center gap-1.5 rounded-md border px-2 py-0.5 text-xs font-mono uppercase font-bold tracking-wider ${statusBadge[assignment.status] ?? ""}`}
              >
            ...
            <span
              className={`inline-flex rounded-md border px-2 py-0.5 text-xs font-mono uppercase font-bold tracking-wider ${priorityBadge[basicTask.priority] ?? ""}`}
            >
              {basicTask.priority}
            </span>
          </div>
      ...
      <div className="grid grid-cols-2 gap-3 bg-slate-950/40 border border-slate-850 p-4 rounded-xl">
        <DetailRow icon={Calendar} label="Target Deadline" value={new Date(basicTask.deadline).toLocaleDateString("en-GB")} />
        <DetailRow icon={Calendar} label="Assigned At" value={assignment ? new Date(assignment.assignedAt).toLocaleDateString("en-GB") : "—"} />
        <DetailRow icon={User} label="Assigned By" value={assignment?.assigner?.fullName ?? "—"} />
        <DetailRow icon={User} label="Support" value={assignment?.support?.fullName ?? "—"} />
        {task && <DetailRow icon={Layers} label="Group" value={task.taskGroup?.name ?? "—"} />}
        {task?.startDate && <DetailRow icon={Calendar} label="Start Date" value={new Date(task.startDate).toLocaleDateString("en-GB")} />}
        {task?.estDays && (
          <div className="col-span-2 mt-1 pt-2 border-t border-slate-850">
            <DetailRow label="Est. Duration" value={`${task.estDays} days`} />
          </div>
        )}
      </div>
      ...
      {basicTask.description && (
        <div className="space-y-1.5">
          <h4 className="text-[10px] font-bold uppercase font-mono tracking-widest text-slate-500">Description</h4>
      ...
      {task?.taskNotes && (
        <div className="space-y-1.5">
          <h4 className="text-[10px] font-bold uppercase font-mono tracking-widest text-slate-500">Notes</h4>
                ...
                >
                  <div className="flex items-center justify-between mb-3">
                    <div className="flex items-center gap-2">
                      ...
                      {isLatest && (
                        <span className="text-[10px] text-cyan-400 font-mono bg-cyan-500/10 border border-cyan-500/30 px-1.5 py-0.5 rounded">
                          LATEST
                        </span>
                      )}
                    </div>
                    <div className="flex items-center gap-2">
                      ...
                      {sub.reviewStatus === "PENDING" && isLatest && (
                        <button
                          onClick={() => onEditSubmission(sub)}
                          className="flex items-center gap-1 rounded-lg border border-amber-400/30 bg-amber-500/10 px-2.5 py-1 text-[11px] font-medium text-amber-300 transition hover:border-amber-400/50 hover:bg-amber-500/20"
                        >
                  ...
                  <div className="space-y-1.5">
                    ...
                    {sub.note && (
                      <p className="text-xs text-slate-400 leading-relaxed">{sub.note}</p>
                    )}
                    {sub.reviewComment && (
                      <div className="mt-2 rounded-lg border border-amber-500/20 bg-amber-500/5 px-3 py-2">
                        ...
                        <p className="text-xs text-amber-300 italic">{sub.reviewComment}</p>
                      </div>
                    )}
                  </div>
                </div>
