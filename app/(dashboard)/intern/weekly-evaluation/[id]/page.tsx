...
function RatingBadge({ level }: { level: RatingLevel }) {
  return (
    <span
      className={`inline-flex items-center px-2.5 py-0.5 text-xs font-semibold rounded-lg border ${RATING_COLORS[level]}`}
    >
      {RATING_LABELS[level]}
    </span>
  );
          ...
          <div className="divide-y divide-border/20">
            {section.criteria.map((criterion, idx) => {
              ...
              return (
                <div
                  key={criterion.key}
                  className="flex items-center gap-4 px-4 py-2.5 hover:bg-white/[0.01]"
                >
                ...
                return (
                  ...
                  >
                    <div className="w-full flex items-end h-10">
                      <div
                        className={`w-full rounded-sm bg-gradient-to-t transition-all duration-500 ${
                          isCurrent
                            ? `${g.color} opacity-100`
                    ...
                    <span
                      className={`text-[9px] font-mono ${
                        isCurrent ? "text-primary-light font-bold" : "text-muted"
                      }`}
                    >
                      W{ev.week}
                    </span>
