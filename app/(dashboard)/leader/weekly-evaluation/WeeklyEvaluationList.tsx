...
export default function WeeklyEvaluationList() {
  ...
  const handleDelete = async (id: string) => {
    if (confirm(t("deleteConfirm"))) {
      try { await deleteEvaluation.mutateAsync(id); } catch (err) { console.error(err); }
    }
  };

  const handlePageChange = (page: number) => {
    const nextParams = new URLSearchParams(searchParams.toString()); nextParams.set("page", String(page)); router.push(`${pathname}?${nextParams.toString()}`);
  };
    ...
    <MetalCard className="p-6">
      <div className="flex flex-col gap-6 sm:flex-row sm:items-center sm:justify-between border-b border-white/10 pb-5 mb-6">
        ...
        <div className="flex items-center gap-3">
          <Modal>
            <Modal.Open opens="create-evaluation"><Button variant="primary" size="md"><Plus className="h-4 w-4 mr-1 inline" />{t("createEvaluation")}</Button></Modal.Open>
          ...
          return (
            <Table.Row key={item.id}>
              ...
              <div className="flex items-center justify-center gap-2">
                <Link href={`/leader/weekly-evaluation/${item.id}`}><Button variant="glass" size="sm" className="flex items-center gap-1"><Eye className="h-3.5 w-3.5 mr-1" /><span>{t("view")}</span></Button></Link>
                <WeeklyEvaluationExportButton id={item.id} />
                <Button variant="danger" size="sm" onClick={() => handleDelete(item.id)} disabled={deleteEvaluation.isPending}><Trash2 className="h-3.5 w-3.5" /></Button>
              </div>
            </Table.Row>
          );
        }} />
        {totalPages > 1 && (
          <Table.Footer>
            <div className="flex items-center gap-4">
              <Button variant="glass" size="sm" onClick={() => handlePageChange(currentPage - 1)} disabled={currentPage <= 1}><ChevronLeft className="h-4 w-4" /></Button>
              <span className="text-sm text-muted">{t("pagination", { page: currentPage, totalPages })}</span>
              <Button variant="glass" size="sm" onClick={() => handlePageChange(currentPage + 1)} disabled={currentPage >= totalPages}><ChevronRight className="h-4 w-4" /></Button>
            </div>
          </Table.Footer>
        )}
      </Table>
