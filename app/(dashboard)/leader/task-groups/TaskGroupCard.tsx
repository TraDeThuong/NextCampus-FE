      ...
      <MetalCard className="p-5 flex flex-col justify-between transition-all duration-300 hover:border-cyan-400/40 hover:shadow-[0_8px_30px_rgba(21,174,245,0.12)]">
            ...
            {members.length > 0 ? (
              <div className="flex flex-wrap gap-1.5">
                {members.slice(0, 4).map((member) => (
                  <span
                    key={member.internId}
                    className="inline-flex items-center rounded-lg bg-white/5 border border-white/5 px-2 py-0.5 text-xs text-slate-300"
                  >
                    {member.intern.fullName}
                  </span>
                ))}
                {members.length > 4 && (
                  <span className="inline-flex items-center rounded-lg bg-white/5 px-2 py-0.5 text-xs text-slate-500">
                    +{members.length - 4}
                  </span>
                )}
              </div>
        ...
        <div className="flex items-center justify-between gap-2 pt-3 border-t border-white/10">
          <Modal.Open opens={`quota-allocation-${group.id}`}>
            <button
              type="button"
              className="flex-1 flex items-center justify-center gap-1.5 rounded-xl border border-cyan-400/30 bg-cyan-500/10 py-2 px-3 text-xs font-semibold text-cyan-300 hover:border-cyan-400/50 hover:bg-cyan-500/20 transition"
            >
          ...
          <div className="flex items-center gap-1 shrink-0">
            <Modal.Open opens={`edit-task-group-${group.id}`}>
              <button
                type="button"
                className="p-2 rounded-xl border border-white/10 bg-white/5 text-slate-300 hover:text-white hover:border-white/20 transition"
...
function DeleteConfirm({
  ...
  return (
    <div className="px-2 py-6 text-center">
      <div className="mx-auto flex h-12 w-12 items-center justify-center rounded-full bg-red-500/10 text-red-400 mb-3">
      ...
      <div className="mt-6 flex justify-center gap-3">
        <button
          ...
          disabled={isDeleting}
          className="rounded-xl border border-white/10 bg-white/5 px-5 py-2 text-xs text-slate-300 hover:text-white transition"
        >
        ...
        <button
          ...
          disabled={isDeleting}
          className="rounded-xl bg-red-600 px-5 py-2 text-xs font-semibold text-white hover:bg-red-500 transition"
        >
          {t("leader.taskGroups.confirm")}
        </button>
      </div>
    </div>
  );
