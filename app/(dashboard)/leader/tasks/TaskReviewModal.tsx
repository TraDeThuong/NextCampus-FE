...
export default function TaskReviewModal({ assignmentId, onClose }: Props) {
  ...
  const reviewSubmissionMutation = useReviewTaskSubmission();

  const assignment = assignmentData?.data;
  const submissions = submissionsData?.data ?? [];
  const latestSubmission = submissions[0];

  const isProcessing = reviewSubmissionMutation.isPending;

  const handleReview = (reviewStatus: ReviewStatus) => {
    if (!latestSubmission) return;

    if (reviewStatus === "REJECTED" && !reviewComment.trim()) {
      setCommentError("Nhận xét lý do cần sửa là bắt buộc khi yêu cầu làm lại.");
      toast.error("Vui lòng nhập nhận xét lý do cần sửa trước khi yêu cầu làm lại.");
      return;
    }
              ...
              {submissionsLoading ? (
                ...
                <div className="rounded-xl border border-white/10 bg-white/[0.03] p-4">
                  ...
                  <textarea
                    ...
                    placeholder="Viết nhận xét đánh giá hoặc lý do yêu cầu sửa đổi..."
                    rows={3}
                    ...
                    {isProcessing ? (
                      ...
                      <>
                        <button
                          ...
                          disabled={isProcessing}
                          className="flex items-center gap-1.5 rounded-xl bg-emerald-600 px-4 py-2.5 text-xs font-semibold text-white transition hover:bg-emerald-500 disabled:opacity-50 shadow-md shadow-emerald-900/20"
                        >
                          <CheckCircle className="h-4 w-4" />
                          Chấp thuận (Approve ➔ DONE)
                        </button>
                        <button
                          type="button"
