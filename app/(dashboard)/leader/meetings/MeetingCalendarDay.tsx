...
interface Props { date: Date | null; meetings: Meeting[]; isToday: boolean; isCurrentMonth: boolean; isPast: boolean; currentUserId?: string; excusedMeetingIds?: Set<string>; onClickMeeting: (meetingId: string) => void; onScheduleClick?: () => void; }
    ...
    <div className={`relative min-h-[100px] rounded-xl border p-1.5 transition ${isToday ? "border-primary-main/40 bg-primary-main/5 shadow-[0_0_12px_rgba(59,130,246,0.15)]" : "border-white/5 bg-white/[0.02] hover:border-white/10"}`}>
      <div className={`mb-1 flex items-center justify-between text-xs font-medium ${isCurrentMonth ? "text-slate-300" : "text-slate-600"}`}>
        ...
        {!isPast && onScheduleClick && (
          <button type="button" onClick={(e) => { e.stopPropagation(); onScheduleClick(); }} className="flex h-5 w-5 items-center justify-center rounded-md text-slate-600 transition hover:bg-primary-main/10 hover:text-primary-light" title={t("scheduleHint")}><Plus className="h-3.5 w-3.5" /></button>
        )}
      </div>
      <div className="space-y-0.5">
        {visibleMeetings.map((meeting) => {
          ...
          return (
            <button key={meeting.id} type="button" onClick={() => onClickMeeting(meeting.id)} className={`w-full truncate rounded border-l-2 px-1.5 py-0.5 text-left text-[10px] leading-tight transition hover:brightness-125 ${baseColor} ${!isHosted ? "border-dashed opacity-75" : ""} ${isExcused ? "opacity-40 saturate-50" : ""}`}>{meeting.title}</button>
          );
        })}
