...
export default function WeekMeetingsCard({
  ...
  function getParticipantLabel(m: Meeting) {
    ...
    for (const p of m.participants) {
      // Only count invited participants, not HOST/ORGANIZER
      if (p.participantRole !== "PARTICIPANT") continue;
      if (internUserIds.has(p.userId)) interns++;
      else leaders++;
    }
    const parts: string[] = [];
    if (interns > 0) parts.push(`${interns} intern${interns !== 1 ? "s" : ""}`);
    if (leaders > 0) parts.push(`${leaders} leader${leaders !== 1 ? "s" : ""}`);
    return parts.length > 0 ? parts.join(", ") : "0 participants";
  }
                            ...
                            <div className="flex items-start gap-2">
                              ...
                              <div className="min-w-0 flex-1">
                                <div className="flex items-center gap-2">
                                  ...
                                  >
                                    {m.createdBy === currentUser?.id ? "Hosted" : "Invited"}
                                  </span>
                                </div>
