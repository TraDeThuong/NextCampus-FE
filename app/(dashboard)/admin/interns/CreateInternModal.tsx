...
import { useCreateIntern } from "@/hooks/intern/useCreateIntern";
...
export default function CreateInternModal({ open, onClose }: Props) {
  ...
  const isPending = creating || createIntern.isPending;
  ...
  function validate(): boolean {
    ...
    else if (!/(0[3|5|7|8|9]|01[2|6|8|9])[0-9]{8}/.test(phone.trim())) e.phone = "Invalid VN phone number";
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const userId = (userRes as any).data?.id ?? (userRes as any).id;
      ...
      const msg = err?.response?.data?.message ?? err?.message ?? "Failed to create intern.";
      toast.error(msg);
    } finally {
      setCreating(false);
    }
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <Field label="Leader">
              ...
              >
                ...
                {leaders.map((l: any) => (
                  <option key={l.id} value={l.id}>{l.user?.fullName ?? l.id}</option>
                ))}
              </select>
            </Field>
