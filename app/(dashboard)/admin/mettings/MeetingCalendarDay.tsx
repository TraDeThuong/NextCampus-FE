...
const INVITED_COLORS: Record<string, string> = {
  SCHEDULED: "bg-amber-500/15 text-amber-300 border-l-amber-500",
  ONGOING: "bg-orange-500/15 text-orange-300 border-l-orange-500",
  COMPLETED: "bg-yellow-500/10 text-yellow-300 border-l-yellow-500",
  CANCELLED: "bg-red-500/20 text-red-300 border-l-red-500",
  DRAFT: "bg-stone-500/15 text-stone-300 border-l-stone-500",
};
  ...
  return (
    ...
    >
      ...
      <div className="space-y-0.5">
        ...
        {overflow > 0 && (
          <p className="px-1 text-[10px] text-slate-500">{t("admin.meetings.moreItems", { n: overflow })}</p>
        )}
      </div>
    </div>
  );
