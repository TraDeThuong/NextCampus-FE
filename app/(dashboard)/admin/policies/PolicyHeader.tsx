...
function AddPolicyForm({
  ...
  const {
    ...
    defaultValues: { title: "", content: "", isActive: false },
  });
  ...
  return (
    <div className="px-2 py-4">
      <div className="flex items-center gap-3 border-b border-white/10 pb-4">
        ...
        <div>
          <h3 className="text-lg font-semibold text-white">{t("admin.policies.createTitle")}</h3>
          <p className="text-xs text-slate-400">{t("admin.policies.createDesc")}</p>
        </div>
      </div>
      ...
      >
        <div className="space-y-1">
          ...
          <input type="text" placeholder={t("admin.policies.titlePlaceholder")} {...register("title")} className={inputClass} />
        ...
        <div className="space-y-1">
          ...
          <Controller control={control} name="content" render={({ field }) => (
            <RichTextEditor value={field.value} onChange={field.onChange} placeholder={t("admin.policies.contentPlaceholder")} />
          )} />
          {errors.content && <p className="text-xs text-red-400">{errors.content.message}</p>}
        </div>

        <div className="flex items-center gap-2 pt-2">
          <input type="checkbox" id="isActive" {...register("isActive")} className="h-4 w-4 rounded border-white/10 bg-white/5 text-cyan-500 focus:ring-0 cursor-pointer" />
          <label htmlFor="isActive" className="text-sm text-slate-300 cursor-pointer select-none">
            {t("admin.policies.activateImmediately")}
          </label>
        </div>
        ...
        <div className="flex justify-end gap-3 pt-4 border-t border-white/5 mt-6">
          ...
          <Button type="submit" disabled={isPending} variant="glass">
            {isPending ? <Loader2 className="h-4 w-4 animate-spin" /> : t("admin.policies.createPolicy")}
          </Button>
        </div>
      </form>
    </div>
