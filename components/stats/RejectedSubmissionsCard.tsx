      ...
      <div className="flex items-start justify-between">
        <div>
          ...
          <h3 className="text-2xl font-black text-red-300 mt-2">
            {rejected.length} Submission{rejected.length !== 1 ? "s" : ""} Requested Rework
          </h3>
          <p className="text-xs text-muted mt-1">
            Your leader has requested changes and resubmission for the following
            tasks
          </p>
        </div>
        ...
        {rejected.slice(0, 3).map((s) => (
          ...
          >
              ...
              <p className="text-muted">
                ...
                {s.reviewComment && (
                  <span className="ml-2 italic">
                    — &quot;{s.reviewComment.slice(0, 60)}
                    {s.reviewComment.length > 60 ? "..." : ""}&quot;
                  </span>
                )}
              </p>
            </div>
            <Link
              ...
              className="ml-3 shrink-0 flex items-center gap-1 text-xs font-medium text-red-400 hover:underline"
            >
              Resubmit <ExternalLink className="h-3 w-3" />
            </Link>
          </div>
        ))}
      </div>
