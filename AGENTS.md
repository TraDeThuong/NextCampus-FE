...
3. **Quy chuẩn UI/UX đã khóa**:
   ...
   - **Chuẩn hóa Component & Form & Table**: Bắt buộc tuân thủ đủ 6 trạng thái thành phần (Normal, Hover, Focus, Active, Disabled, Loading), xử lý vỡ bảng (overflow, fixed/flex width, scroll/card list mobile, sticky header), menu giới hạn `max-h-60`, form hiển thị viền đỏ + icon + text lỗi rõ ràng theo [`.agents/rules/ui-ux-checklist.md`](file:///d:/NodeJS/NexCampus/NexCampus-FE/.agents/rules/ui-ux-checklist.md). **Đặc biệt: Các ô nhập liệu (`Input`, `Select`, `DatePicker`, `TimePicker`, `DateTimePicker`, radio card buttons...) khi nằm trên cùng một hàng BẮT BUỘC phải có chiều cao bằng nhau tuyệt đối (`h-[42px] sm:h-[46px]`, padding `px-4 py-2.5 sm:py-3 text-sm rounded-xl`)**, nhãn label dùng chung `text-xs sm:text-sm font-medium text-foreground/90 select-none flex items-center gap-1` và khoảng cách `gap-1.5`. Tuyệt đối không để ô cao ô thấp lệch hàng lối. **Đặc biệt với Bảng Dữ Liệu: Tuyệt đối KHÔNG bọc thêm thẻ `<MetalCard>` bên ngoài `<Table>`** (như chuẩn tại `/leader/department` và `/leader/weekly-evaluation`) vì bản thân `<Table>` đã sở hữu container bo góc `rounded-3xl`, viền `border border-border`, bóng kính `shadow-glass` và cuộn ngang; việc bọc thêm `<MetalCard>` sẽ tạo viền kép ngoài (double border) gây lãng phí padding và thu hẹp các cột.
   ...
   - **Quy Chuẩn Thẻ Bộ Lọc Tinh Gọn (Clean & Minimalist Filter Card Layout)**:
     * **Tuyệt đối KHÔNG đặt tiêu đề hay nút Làm mới dữ liệu bên trong thẻ bộ lọc**: Thẻ bộ lọc (`MetalCard` Filter) tuyệt đối không được tự ý thêm các thanh tiêu đề (như *"Phòng ban & Trạng thái"*) hay nút Làm mới dữ liệu (`Table.ReloadButton`) bên trong card làm phân tán và cồng kềnh giao diện.
     * **Bố cục chuẩn**: Thẻ bộ lọc chỉ chứa trực tiếp lưới các ô điều khiển (Search Input, `FilterSelect`...). Nút làm mới dữ liệu chuẩn (`Table.ReloadButton`) nếu có bảng dữ liệu thì đặt tại header cột thao tác của Table, không đặt vào card filter.
     * **Nút Đặt lại bộ lọc (`RotateCcw`)**: Chỉ xuất hiện tại đường viền chân thẻ card (`border-t border-border/40 pt-3`) khi có ít nhất 1 filter/search đang hoạt động (`hasFilters`), giữ cho trạng thái mặc định luôn tinh gọn, thoáng đãng.
   ...
   - **Quy Chuẩn Chọn Giờ (Standardized TimePicker)**:
     * **Tuyệt đối cấm `<input type="time">` native**: Không bao giờ dùng thẻ native `<input type="time">` thô cứng của trình duyệt ở bất kỳ form nào (như cài đặt hạn chót, cấu hình ca trực...).
     * **Bắt buộc dùng `TimePicker`**: Luôn nhập khẩu từ `@/components/ui/TimePicker` hoặc `@/components/ui/DatePicker`.
     * **Tiêu chuẩn UI/UX**:
       - Chiều cao trigger chuẩn `h-[42px] sm:h-[46px]`, bo góc `rounded-xl`, icon `Clock` phát sáng khi mở/có giá trị.
       - Popover hiển thị qua `createPortal(..., document.body)` với `zIndex: 9999`, tự động lật `flip` và kẹp viewport.
       - Hai cột trượt mượt mà: Giờ (`00`-`23`) và Phút (`00`-`59` theo `step`), tự động scroll tới vị trí đang chọn, thanh cuộn chuẩn `.scrollbar-dropdown` (5px, cyan glow).
       - Tích hợp thanh phím tắt chọn giờ nhanh (`presets`: 08:00, 09:00, 12:00, 13:30, 17:30, 21:00...) và nút chọn "Bây giờ".
       - Lưu trữ và gửi API chuẩn 24h `HH:mm`.
       - Đầy đủ 6 trạng thái UI/UX, hỗ trợ `label`, `required`, `error`, `helperText`, nút xóa nhanh `X`.
   - **Quy Chuẩn Chọn Ngày & Giờ (Standardized DateTimePicker)**:
     * **Tuyệt đối cấm `<input type="datetime-local">` native**: Không dùng `<input type="datetime-local">` native ở bất kỳ trang quản trị, bảo trì hay cấu hình API key.
     * **Bắt buộc dùng `DateTimePicker`**: Luôn nhập khẩu từ `@/components/ui/DateTimePicker` hoặc `@/components/ui/DatePicker`.
     * **Tiêu chuẩn UI/UX**:
       - Chiều cao trigger chuẩn `h-[42px] sm:h-[46px]`, bo góc `rounded-xl`, icon `Calendar` + `Clock`.
       - Popover gắn qua `createPortal(..., document.body)` với `zIndex: 9999`.
       - Bố cục thích ứng linh hoạt: Trên Desktop (`≥ 640px`) hiển thị song song (Side-by-side) lịch 7 ngày và 2 cột giờ/phút; trên Mobile (`< 640px`) chuyển sang tab chuyển đổi mượt mà `[ 📅 Ngày ]` và `[ ⏰ Giờ ]` chống tràn viền.
       - Thanh phím tắt chọn nhanh: "Bây giờ", "+1 Giờ", "Hôm nay 18:00", "Hôm nay 23:59", "Ngày mai 09:00"...
       - Lưu trữ và gửi API chuẩn ISO / `YYYY-MM-DDTHH:mm`. Hiển thị người dùng chuẩn `DD/MM/YYYY HH:mm` (vi) hoặc `MM/DD/YYYY HH:mm` (en).
       - Đầy đủ 6 trạng thái UI/UX, hỗ trợ `label`, `required`, `error`, `helperText`, nút xóa nhanh `X`.
   - **Quy Chuẩn Chống Tràn Màn Hình Dropdown & Popover Trên Mobile**:
     * **Nguyên tắc**: Mọi Dropdown / Popover (đặc biệt là `DatePicker`, `DateRangePicker`, `TimePicker`, `DateTimePicker`, `NotificationDropdown`, `FilterSelect`) trên giao diện di động (`< sm`) **TUYỆT ĐỐI KHÔNG để rơi vào tình trạng tràn mép trái/phải hoặc mất hút dưới đáy màn hình**.
     ...
     * **DatePicker / DateRangePicker / TimePicker / DateTimePicker trên Mobile**: Tự động tính toán chiều rộng responsive `Math.min(340, window.innerWidth - 20)` và căn giữa màn hình (`Math.round((vw - popoverWidth) / 2)`). Chiều cao tự động kẹp trong viewport `max-h-[calc(100dvh-20px)] overflow-y-auto`. Hàng presets chọn nhanh phải cuộn ngang (`overflow-x-auto`) thay vì wrap nhiều dòng đẩy lịch xuống dưới.
